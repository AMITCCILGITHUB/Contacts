...
public class MessagePopUp {
    ...
    private SimpleDoubleProperty wrapWidth = new SimpleDoubleProperty( DEFAULT_WIDTH - 58d );
    private HBox actionBox;
    StackPane mask = StackPaneBuilder.create().style( "-fx-background-color:black; -fx-opacity:.2; -fx-background-radius: 10px;" ).build();

    public MessagePopUp( Stage parentStage, String title, String type, String message, double width ) {
        this( parentStage, title, type, message );
        setWrapWidth( width );
    }

    public MessagePopUp( final Stage parentStage, String title, String type, String message ) {
        this.root = new StackPane();
        this.root.autosize();

        this.scene = new Scene( root, DEFAULT_WIDTH, DEFAULT_HEIGHT, Color.web( "#FFFFFF" ) );
        this.scene.getStylesheets().add( Lorens.class.getResource( "/org/map/style/popup.css" ).toExternalForm() );
        ...
        if ( type.equalsIgnoreCase( "Error" ) ) {
            icon = ImageViewBuilder.create().image( new Image( Lorens.class.getResourceAsStream( "/org/map/images/error.png" ) ) ).build();
        }
        else if ( type.equalsIgnoreCase( "Info" ) ) {
            icon = ImageViewBuilder.create().image( new Image( Lorens.class.getResourceAsStream( "/org/map/images/info.png" ) ) ).build();
        }
        else if ( type.equalsIgnoreCase( "Confirm" ) ) {
            icon = ImageViewBuilder.create().image( new Image( Lorens.class.getResourceAsStream( "/org/map/images/confirm.png" ) ) ).build();
        }
        else {
            icon = ImageViewBuilder.create().image( new Image( Lorens.class.getResourceAsStream( "/org/map/images/info.png" ) ) ).build();
        ...
        parentStage.yProperty().addListener( new ChangeListener<Object>() {
            ...
            @Override
            public void changed( ObservableValue<?> arg0, Object arg1, Object arg2 ) {
                stage.setY( Double.valueOf( arg2.toString() ) + parentStage.getScene().getHeight() + 20 );
            }
        } );
    }
